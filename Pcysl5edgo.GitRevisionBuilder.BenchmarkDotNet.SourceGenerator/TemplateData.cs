...
internal readonly struct TemplateData(string commitId, string alias, bool baseLine, string? description, int operationsPerInvoke, string rewrittenText) : IEquatable<TemplateData>, IComparable<TemplateData>
{
    ...
    public bool Equals(TemplateData other)
        => CommitId == other.CommitId && Alias == other.Alias
    ...
    {
        return ((OperationsPerInvoke << 1) | (BaseLine ? 1 : 0)) ^ CommitId.GetHashCode() ^ Alias.GetHashCode() ^ (Description?.GetHashCode() ?? 0);
    }
}
